...
class DashboardView(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self._last_refresh_lbl = ctk.CTkLabel(
            header, text="", font=ctk.CTkFont(size=11), text_color="#555577")
        self._last_refresh_lbl.grid(row=1, column=0, columnspan=2, sticky="w")

        # ── Getting Started card (row=1, hidden by default) ──
        self._gs_frame = ctk.CTkFrame(self, corner_radius=10, fg_color="#12122a",
        ...
        self._cards["defender"] = self._make_card(
            grid, 0, 0, "Security Posture", "—", [], "#1f3355",
            lambda: self._navigate("winsec"))
        self._cards["signatures"] = self._make_card(
            grid, 0, 1, "K2 Engine Signatures", "—", [], "#1f3355",
            lambda: self._navigate("update"))
        self._cards["last_scan"] = self._make_card(
            grid, 0, 2, "Last Scan", "—", [], "#1f3355",
            lambda: self._navigate("scan"))
        self._cards["watcher"] = self._make_card(
            grid, 0, 3, "Folder Watcher", "—", [], "#1f3355",
            lambda: self._navigate("watcher"))

        # ── Quick actions ──
        actions = ctk.CTkFrame(self, corner_radius=10, fg_color="#1a1a2e")
        ...
        for col, (label, cmd) in enumerate(quick_btns):
            ctk.CTkButton(actions, text=label, height=34,
                          fg_color="#1f3355", hover_color="#144e7a",
                          font=ctk.CTkFont(size=12),
                          command=cmd).grid(
                row=1, column=col, padx=8, pady=(0, 14), sticky="ew")
        ...
        self._threats_frame = ctk.CTkScrollableFrame(
            self, height=140, corner_radius=8, fg_color="#1a1a2e")
    ...
    def _apply(self, dfn_status, sig_info, last_scan, threats, score_info=None):
        ...
        self._update_card("signatures", f"{sig_count} files", "#cdd6f4", [
    ...
    def _build_getting_started(self, has_db: bool, has_svc: bool,
        ...
        for i, (done, title, desc, nav_key) in enumerate(steps):
            ...
            icon  = "✓" if done else "○"
            color = "#50fa7b" if done else "#aaaacc"
            ...
            ctk.CTkLabel(row_f, text=title,
                         ...
                         text_color="#666688" if done else "#cdd6f4",
