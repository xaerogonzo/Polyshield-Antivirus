...
**Docs:** [ARCHITECTURE.md](docs/ARCHITECTURE.md) · [WINDOWS_SERVICE.md](docs/WINDOWS_SERVICE.md) · [TESTING.md](docs/TESTING.md) · [CLAUDE.md](CLAUDE.md)
...
For the Guardian AI 4-tier scan pipeline, performance benchmarks, database schema, file structure, threading patterns, and architectural notes → **[ARCHITECTURE.md](docs/ARCHITECTURE.md)**.
...
For the full technical explanation of how this was built — the pywin32 venv incompatibility discovery, the `_exe_name_` fix, the ACL setup, the push-event socket protocol — see **[WINDOWS_SERVICE.md](docs/WINDOWS_SERVICE.md)**.
...
For the full testing guide — including the **Windows Sandbox workflow**, **EICAR sprint stress test**, **Ghost Connection network test**, and **Service Recovery test** — see **[TESTING.md](docs/TESTING.md)**.
...
Full schema → **[ARCHITECTURE.md — Database Schema](docs/ARCHITECTURE.md#database-schema)**.
...
| | | |
| `config/service_events.json` | 🔄 Runtime | Persisted threat events from the Windows Service (atomic writes) |
| `C:\ProgramData\PolyShield\` | 📦 Installer | Service data dir — created by `setup_service.bat` in `scripts\service\` |
| `C:\ProgramData\PolyShield\service_token.txt` | 📦 Installer | UUID4 shared secret — generated by service on first start |
| `C:\ProgramData\PolyShield\service.log` | 🔄 Runtime | Service log (written by the service process) |
...
Full annotated file tree → **[ARCHITECTURE.md — File Structure](docs/ARCHITECTURE.md#file-structure)**.
...
Processes running as SYSTEM or under Protected Process Light (lsass.exe, MsMpEng.exe, DRM services) deny read access to their executable — the MD5 hash cannot be computed. These are silently skipped. See [ARCHITECTURE.md — Known Limitations](docs/ARCHITECTURE.md).
...
The current distribution is a Python source tree with a `kicomav_env\` venv. Pros: easy to update and debug. Cons: bytecode is easily extractable, startup is slower, pywin32 service registration is fragile.

**Why Nuitka over PyInstaller:**

| | PyInstaller | Nuitka |
|---|---|---|
| Output | Bytecode bundle | Compiled C → native .exe |
| Extractability | `pyinstxtractor` trivially reverses it | C compilation; no bytecode |
| DLL handling | Bundles everything; fragile | Clean DLL references |
| pywin32 service | `_exe_name_` fix still needed | Points to compiled .exe directly |
| Startup time | ~2–4 sec (Python bootstrap) | ~0.2 sec (native) |

**Planned build steps:**
```
nuitka --standalone --onefile --windows-disable-console
       --include-package=customtkinter
       --include-package=kicomav_env.Lib.site-packages
       src/ui/app.py
```

**`_exe_name_` in `kicomai_service.py`** — currently set to `sys.executable` (the kicomav_env Python). After Nuitka compilation it would point to the compiled `app.exe`. The service class would need to specify the compiled binary explicitly.

**Timing:** Target this after the feature set stabilizes (post-network layer). Iterating on compiled code is painful; keep source-based until feature work is complete.
...
2. **Testing** — No automated tests yet; see [TESTING.md](docs/TESTING.md) for manual test procedures
...
| **[ARCHITECTURE.md](docs/ARCHITECTURE.md)** | Detection layers, scan pipelines, performance, DB schema, file structure, threading patterns |
| **[WINDOWS_SERVICE.md](docs/WINDOWS_SERVICE.md)** | Service implementation deep-dive, IPC protocol, pywin32 war story, ACL design |
| **[TESTING.md](docs/TESTING.md)** | Testing procedures, battlespace tests, Windows Sandbox workflow, service log reference |
