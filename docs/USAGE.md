...
- [x] **v1.16 — Installer** (`PolyShield-Setup-1.16.0.exe`, 52 MB. Installs the program, creates its data directory with the right permissions, registers the service and adds the Explorer menu entry. Uninstalling removes all of that and **keeps your quarantine, logs and settings** unless you tick the box asking it not to. Verified on a clean machine: 47 checks covering install, reinstall over a broken previous attempt, and uninstall.)
- [x] **v1.16 — K2 arrives with its full signature set** (K2 keeps about 23 signatures inside the program itself and downloads the other ~1,240. If that download did not happen — no connection during setup, say — it reported success anyway, leaving a scanner at under 2% of its detection with nothing saying so. Setup now waits for a connection, counts how many signatures K2 can actually name afterwards, and writes down what happened. The Update Center shows that count too, and says when only the built-in ones are present.)
